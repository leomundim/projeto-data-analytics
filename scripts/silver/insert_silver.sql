...
SELECT
	...
	CASE 
		WHEN Cliente LIKE '%indústria%' OR Cliente LIKE '%fábrica%' THEN 'Indústria'
		WHEN CLiente LIKE '%restaurante%' OR Cliente LIKE '%gastro%' OR Cliente LIKE '%forno%' THEN 'Restaurante'
		WHEN Cliente LIKE '%padaria%' OR Cliente LIKE '%pão%' THEN 'Padaria Artesanal'
		WHEN Cliente LIKE '%torta%' OR Cliente LIKE '%trufa%' OR Cliente LIKE '%confeitaria%' THEN 'Torteria'
		WHEN Cliente LIKE '%café%' OR Cliente LIKE '%bistrô%' THEN 'Café & Bistrô'
		ELSE 'Café & Bistrô'
	END AS segmento, -- Ajustando os segmentos baseados no nome do cliente, uma vez que na camada bronze os dados vieram misturados.
	id_produto,
	produto,
